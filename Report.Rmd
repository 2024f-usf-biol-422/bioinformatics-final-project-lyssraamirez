...
# Figures
```{r barplot-SNP_gene-distribution, echo=TRUE, warning=FALSE, message=FALSE, fig.width=10, fig.height=8}
...
```{r figure-1,normalized_sequence_coverage, echo=TRUE, warning=FALSE, message=FALSE}
...
normalized_plot <- vcf_with_metadata %>%
  mutate(Bases_per_person = Bases / ww_population) %>%  # Corrected mutate
...
```{r-Population_size_AVGspotlens_regression_line, echo=FALSE}
...
**Figure 4:** Box plot showing the distribution of sequencing base counts for different sample matrices ('post grit removal' and 'raw wastewater'), categorized by sampling methods ('composite' and 'grab'). The median, interquartile range, and outliers are displayed, highlighting how the sampling matrix and method impact the RNA yield, which could influence SARS-CoV-2 detection accuracy.


```{r summary-table, echo=FALSE}
ggplot(summary_table, aes(x = ww_sample_matrix, y = Mean_Coverage, fill = ww_sample_type)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  labs(
    title = 'Mean Coverage Comparison by Sample Type and Matrix',
    x = 'Sample Matrix',
    y = 'Mean Coverage (Bases)',
    fill = 'Sample Type'
  ) +
  theme_minimal()
```
...
summary_table <- data %>%
  ...
  summarize(
    ...
    Sample_Count = n(),
    .groups = "drop" # Suppresses regrouping warning
  )
