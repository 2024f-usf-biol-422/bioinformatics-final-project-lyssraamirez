...
Visualizations and analyses were conducted using R programming. R packages that were used are data manipulation are `ggplot2` [@2016ggplot2], `dplyr` [@2023dplyr] and `tidyverse` [@Rtidyverse]. `Rmarkdown` was used for report generation and rendering Markdown files into HTML and pdf. [@Rrmarkdown] `ggthemes`[@2022ggthemes] was used to create figures illustrating SNP distribution, temporal trends, and relationships between key variables, such as population size and RNA detection levels. `RColorBrewer`[@RColorBrewer] was used hand in hand with `ggthemes` to create colors palettes in the figures.
