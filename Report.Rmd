...
Visualizations and analyses were conducted using R programming. R packages that were used are data manipulation are `ggplot2` [@2016ggplot2], `dplyr` [@2023dplyr] and `tidyverse` [@Rtidyverse]. `Rmarkdown` was used for report generation and rendering Markdown files into HTML and pdf. [@Rrmarkdown] The R package `ggthemes`[@2022ggthemes] was used to create figures illustrating SNP distribution, temporal trends, and relationships between key variables, such as population size and RNA detection levels. To create color on the figures `RColorBrewer`[@RColorBrewer] was used hand in hand with `ggthemes`. The R package `magrittr` [@magrittrwas] used to utilize the pipe operator in code. To manipulate and visualize variant call format (VCF) files, `vcfR`[@vcfR] was used. To enable report generation and integration of R code `knitr` was used.[@knitr] To make sure all of the pipeline worked correctly `testthat` was used.[@testthat]
...
The analysis of composite and grab samples showed the distinct differences in RNA detection consistency and data accuracy.Composite samples, which were collected over the course of 24 hours, provided a higher average in sequencing coverage compared to the grab samples.[@2022Kmush] As showed in Figure.5, the distribution of sampling methods showed a drastic difference in count. Composite sampling minimizes temporal variability which results in a overall higher value in representative number of SARS-CoV-2 RNA levels. On the other hand, grab samples collect more at a single time but show a major deduction in sequencing coverage and high variability. Which is likely due to the temporal flunctuations of viral RNA levels in wastewater.[@2022Kmush]

#### Temporal Trends in Sequencing Coverage
Temporal fluctuations in sequencing coverage were observed across the study period (Figure 2). Peaks in sequencing coverage corresponded to periods of increased viral prevalence in the Pennsylvania region, suggesting alignment with SARS-CoV-2 case surges. This highlights the utility of wastewater surveillance in tracking real-time epidemiological trends. However, certain months exhibited reduced coverage, potentially reflecting reduced viral shedding or challenges in sample processing.

#### Population Size and RNA Detection Levels
A positive correlation between population size and RNA detection levels was observed (Figure 3), suggesting that larger populations contribute to more consistent viral RNA signals in wastewater. However, this correlation is nuanced, as population size may also dilute viral RNA concentration due to increased wastewater volume, a factor that requires further investigation. Interestingly, Figure 4 showed a slight negative correlation between population size and average sequencing read length, suggesting that higher population complexity could influence RNA fragmentation during processing.

#### Genomic Variability Across SARS-CoV-2 Genes
SNP analysis revealed significant variability across SARS-CoV-2 genes, with the S gene exhibiting the highest SNP count (Figure 1). This finding is consistent with the critical role of the spike protein in viral infectivity and host interaction, making it a hotspot for mutations. This observation aligns with global studies on SARS-CoV-2 evolution, emphasizing the need for continued surveillance of the S gene to monitor emerging variants.

#### Variant Distributions in Pennsylvania
The distribution of reference and alternate genome variants (Figure 6) showed a predominance of thymine (T) as the most frequent alternate genome nucleotide. This finding may reflect regional patterns in viral evolution, further underscoring the importance of localized genomic surveillance.

#### Summary of Sequencing Coverage by Sampling Method
Table 1 summarizes the sequencing coverage for different sampling matrices and methods. Composite samples consistently outperformed grab samples in sequencing coverage, reinforcing their reliability for wastewater surveillance.



composit sampling-This method reduces temporal variability and gives a more representative dataset of viral RNA levels over time[@2022augustosampling]
# Discussionn
