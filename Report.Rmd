...
# Filter dataset to only include common variants (e.g., variants with more than 500 occurrences)
common_variants <- common_variants %>%
  filter(Count > 500)

# Create a bar plot to visualize the reference vs. alternate genome occurrences for common variants
ggplot(common_variants, aes(x = ref, y = Count, fill = alt)) +
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10) # Combine rotation and size adjustment
  )
...
# Create a data frame for easy conversion to a table
summary_table_df <- as.data.frame(summary_table)

# Create a table using ggplot
table_plot <- tableGrob(summary_table_df)

# Save the table plot as a PNG file
output_file <- "summary_table_output.png"
ggsave(filename = output_file, plot = table_plot, width = 10, height = 8, dpi = 300, device = "png")
