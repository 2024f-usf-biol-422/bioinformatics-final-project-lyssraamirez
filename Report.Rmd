...
The PAWWS  was then converted into a sraRuntable to be able to call on in the code. R studio packages were utilized, including “vcfR”, “ggplot2”, “ggthemes”, “knitr” and “dplyr”[@dplyrpkg] were used to visualize the data. Figures and tables were generated to illustrate SNP distribution, population trends, temporal patterns, and relationships between variables. Chat Gpt was also used to write the code of figures in this study.
...
# Create the bar plot
snp_plot <- ggplot(gene_snp_summary, aes(x = reorder(gene, -total_snps), y = total_snps, fill = gene)) +
...
library(ggplot2)   # For saving the plot
...
# Convert the table into a graphical table
summary_table_grob <- tableGrob(summary_table)

# Save the table as a PNG
output_file <- "summary_table_output.png"
ggsave(filename = output_file, plot = summary_table_grob, width = 10, height = 8, dpi = 300, device = "png")
```
**Table 1**: 
