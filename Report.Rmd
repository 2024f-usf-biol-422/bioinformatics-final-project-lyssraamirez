...
Visualizations and analyses were conducted using R programming. R packages that were used are data manipulation are `ggplot2` [@2016ggplot2], `dplyr` [@2023dplyr] and `tidyverse` [@Rtidyverse]. `Rmarkdown` was used for report generation and rendering Markdown files into HTML and pdf. [@Rrmarkdown] The R package `ggthemes`[@2022ggthemes] was used to create figures illustrating SNP distribution, temporal trends, and relationships between key variables, such as population size and RNA detection levels. To create color on the figures `RColorBrewer`[@RColorBrewer] was used hand in hand with `ggthemes`. The R package `magrittr` [@magrittrwas] used to utilize the pipe operator in code. To manipulate and visualize variant call format (VCF) files, `vcfR`[@vcfR] was used. To enable report generation and integration of R code `knitr` was used.[@knitr] Lastly, to make sure all of the pipeline worked correctly `testthat` was used.[@testthat]


Figures and tables were generated to effectively summarize and interpret the data. OpenAI's ChatGPT was used to assist in drafting and refining the R code for figure and table generation. All outputs produced by ChatGPT were reviewed, validated, and adjusted to ensure alignment with the study's objectives and data integrity.
