...
How does the prevalence of SARS-CoV-2 in wastewater samples correlate with population size, sampling methods, and changes over time?
...
data$ww_population <- as.numeric(data$ww_population)
data$AvgSpotLen <- as.numeric(data$AvgSpotLen)

# Filter rows with missing or invalid data
filtered_data <- data %>%
  filter(!is.na(ww_population), !is.na(AvgSpotLen))

# Create the scatter plot and assign it to a variable
scatter_plot <- ggplot(filtered_data, aes(x = ww_population, y = AvgSpotLen)) +
  geom_point(alpha = 0.7, color = "blue") +
...
ggsave("population_vs_avgspotlen.png", plot = scatter_plot, width = 10, height = 6, dpi = 300)
